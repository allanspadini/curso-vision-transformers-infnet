# Roteiro de Narração Slide a Slide (Falas do Apresentador)
## Disciplina: Visão Computacional com CNNs e Transformers — Aula 4
**Tema Central:** *Vision Transformers (ViT) e Classificação de Imagens: Fatiamento em Patches, Atenção Global e Regularização com CutMix*  
**Professor:** Prof. Allan Spadini  
**Instituição:** Faculdade Infnet — Pós-Graduação em Inteligência Artificial & Machine Learning  

---

### Slide 1: Vision Transformers (ViT) e Classificação Visual
**Tipo:** `title`  
**Título:** Vision Transformers (ViT) e Classificação Visual  
**Subtítulo:** Da Tirania do Pixel ao Fatiamento em Patches, Atenção Global e Regularização com CutMix  
**Falas do Apresentador:**
> Olá a todos e sejam muito bem-vindos à nossa Aula 4 da disciplina de Visão Computacional com CNNs e Transformers da Faculdade Infnet!
>
...
### Slide 2: A Tirania do Pixel e a Explosão da Autoatenção
**Tipo:** `visual-component` (`PixelAttentionExplosionDiagram`)  
**Título:** A Tirania do Pixel e a Explosão da Autoatenção  
**Subtítulo:** Por que não podemos aplicar o mecanismo de atenção diretamente em cada pixel da imagem?  
**Falas do Apresentador:**
> Vamos começar rigorosamente pela nossa metodologia pedagógica: a Situação-Problema do Mundo Real. 
>
...
### Slide 3: O Dilema do Viés Indutivo: CNN vs Transformer
**Tipo:** `visual-component` (`InductiveBiasTradeoffDiagram`)  
**Título:** O Dilema do Viés Indutivo: CNN vs Transformer  
**Subtítulo:** Premissas estruturais rígidas de localidade versus liberdade total de relações espaciais  
**Falas do Apresentador:**
> Para entender a barreira conceitual entre CNNs e Transformers, precisamos discutir um dos conceitos mais importantes do aprendizado de máquina moderno: o Viés Indutivo, ou Inductive Bias.
>
...
### Slide 5: 🧪 Lab 1: Simulador de Patches e Complexidade
**Tipo:** `interactive` (`PatchSlicingLab`)  
**Título:** 🧪 Lab 1: Simulador de Patches e Complexidade  
**Subtítulo:** Explore a relação entre resolução da imagem, tamanho do patch (P) e custo computacional  
**Falas do Apresentador:**
> Neste primeiro laboratório interativo, convido vocês a experimentarem os trade-offs de fatiamento de imagens.
>
...
### Slide 6: A Macro-Arquitetura do Vision Transformer (ViT)
**Tipo:** `visual-component` (`ViTCanonicalArchitectureViewer`)  
**Título:** A Macro-Arquitetura do Vision Transformer (ViT)  
**Subtítulo:** O pipeline canônico completo: fatiamento, projeção linear, codificador e cabeça densa  
**Falas do Apresentador:**
> Na tela, temos o diagrama mais icônico da visão computacional moderna: a arquitetura canônica do ViT apresentada na Figura 8-11 do livro clássico de Deep Learning.
>
...
### Slide 7: Patch Embedding: Achatamento e Projeção Linear
**Tipo:** `visual-component` (`PatchEmbeddingMechanicsDiagram`)  
**Título:** Patch Embedding: Achatamento e Projeção Linear  
**Subtítulo:** Como transformar matrizes 3D de pixels em vetores latentes D = 768 via álgebra ou convolução strided  
**Falas do Apresentador:**
> Vamos agora mergulhar na matemática do bloco rosa da imagem anterior: a camada de Patch Embedding.
>
...
### Slide 8: O Token [CLS] e os Position Embeddings 1D
**Tipo:** `visual-component` (`ClsTokenPositionalDiagram`)  
**Título:** O Token [CLS] e os Position Embeddings 1D  
**Subtítulo:** Injeção de ordem espacial e o papel do vetor agregador global herdado do BERT  
**Falas do Apresentador:**
> Temos agora os 196 patches projetados como vetores de dimensão 768. Mas antes de alimentá-los no Transformer Encoder, precisamos resolver dois problemas fundamentais:
>
...
### Slide 9: Anatomia do Bloco Transformer Encoder
**Tipo:** `visual-component` (`ViTBlockAnatomyDiagram`)  
**Título:** Anatomia do Bloco Transformer Encoder  
**Subtítulo:** Pre-LayerNorm, Multi-Head Self-Attention, conexões residuais e expansão 4× no MLP  
**Falas do Apresentador:**
> Uma vez que o tensor z_0 com formato [Batch, 197, 768] está montado, ele atravessa uma pilha de 12 blocos Transformer idênticos (no ViT-Base). Vamos analisar a anatomia de um único bloco no diagrama da tela.
>
...
### Slide 10: 🧪 Lab 2: Rastreador Dimensional de Tensores
**Tipo:** `interactive` (`ViTTensorTrackerLab`)  
**Título:** 🧪 Lab 2: Rastreador Dimensional de Tensores  
**Subtítulo:** Acompanhe passo a passo o formato exato dos tensores em cada etapa do forward pass do ViT  
**Falas do Apresentador:**
> No nosso segundo laboratório interativo, colocamos na prática a regra de ouro da nossa disciplina: rastrear rigorosamente as dimensões dos tensores do PyTorch em cada etapa do fluxo.
>
...
### Slide 11: A Situação-Problema: A Fome de Dados do ViT
**Tipo:** `visual-component` (`DataHungerOverfittingDiagram`)  
**Título:** A Situação-Problema: A Fome de Dados do ViT  
**Subtítulo:** Por que treinar um Vision Transformer do zero em datasets moderados (ImageNet-1k) fracassa?  
**Falas do Apresentador:**
> Chegamos agora à nossa segunda grande Situação-Problema do Mundo Real: o paradoxo do treinamento do Vision Transformer.
>
...
### Slide 12: A Sacada do CutMix: Quebrando o Overfitting
**Tipo:** `visual-component` (`CutMixConceptDiagram`)  
**Título:** A Sacada do CutMix: Quebrando o Overfitting  
**Subtítulo:** Forçando o ViT a correlacionar múltiplos patches sem gerar artefatos artificiais nos pixels  
**Falas do Apresentador:**
> Para quebrar a fome de dados do ViT e permitir seu treinamento com sucesso em datasets moderados (conforme demonstrado no paper do DeiT da Meta), os pesquisadores recorreram a técnicas avançadas de Regularização de Dados, com destaque absoluto para o CutMix!
>
...
### Slide 13: Interpretabilidade: Mapas de Atenção do ViT
**Tipo:** `visual-component` (`AttentionRolloutDiagram`)  
**Título:** Interpretabilidade: Mapas de Atenção do ViT  
**Subtítulo:** Visualizando a autoatenção entre o token [CLS] e os patches espaciais (Attention Rollout)  
**Falas do Apresentador:**
> Uma das características mais extraordinárias do Vision Transformer em relação às CNNs é a sua explicabilidade e interpretabilidade nativa.
>
...
### Slide 14: A Família ViT: Configurações, Parâmetros e Desempenho
**Tipo:** `visual-component` (`ViTArchitecturesComparisonDiagram`)  
**Título:** A Família ViT: Configurações, Parâmetros e Desempenho  
**Subtítulo:** Comparativo técnico entre variantes ViT-Base, ViT-Large, ViT-Huge, ResNet e ConvNeXt  
**Falas do Apresentador:**
> Neste slide, sintetizamos a taxonomia completa da família Vision Transformer comparada com os marcos das Redes Convolucionais modernas no ImageNet-1k.
>
...
### Slide 15: 🧪 Lab 3: Explorador de Trade-offs (ViT vs CNN)
**Tipo:** `interactive` (`ViTModelTradeoffLab`)  
**Título:** 🧪 Lab 3: Explorador de Trade-offs (ViT vs CNN)  
**Subtítulo:** Compare acurácia, latência de inferência, uso de VRAM e throughput para tomada de decisão em produção  
**Falas do Apresentador:**
> No nosso terceiro laboratório interativo, colocamos vocês na cadeira de um Arquiteto de Soluções de Inteligência Artificial para responder à pergunta clássica de reuniões de engenharia: 'Qual modelo devemos colocar em produção no nosso projeto?'
>
...
### Slide 16: Quiz Interativo de Fixação
**Tipo:** `quiz` (`ViTInteractiveQuiz`)  
**Título:** Quiz Interativo de Fixação  
**Subtítulo:** Teste seu domínio sobre fatiamento de patches, viés indutivo e regularização com CutMix  
**Falas do Apresentador:**
> Chegamos ao encerramento da nossa aula teórica com o nosso Quiz Interativo de Fixação de Conceitos da Aula 4!
>
