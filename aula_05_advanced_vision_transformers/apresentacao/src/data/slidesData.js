/**
 * slidesData.js - Matriz central de dados da Aula 05: Modelos ViT Avançados
 * Disciplina: Visão Computacional com CNNs e Transformers - Faculdade Infnet
 */

export const slides = [
  // =========================================================================
  // BLOCO 1: REVISÃO PROFUNDA DO ViT CANÔNICO E SUAS LIMITAÇÕES PRÁTICAS (Slides 1 a 4)
  // =========================================================================
  {
    id: 1,
    type: 'title',
    title: 'Modelos ViT Avançados: DeiT, PVT, Swin Transformer & DINO',
    subtitle: 'Revisão do ViT Canônico, Destilação com Tokens, Pirâmides Espaciais, Janelas Deslocadas e Auto-supervisão',
    category: 'Módulo 5 • Transformers em Visão Avançado',
    tag: 'Aula 05',
    badges: [
    ...
    notes: `Olá a todos e sejam muito bem-vindos à nossa Aula 5 da disciplina de Visão Computacional com CNNs e Transformers da Faculdade Infnet!
...
Em seguida, entenderemos as dores práticas enfrentadas pelos engenheiros ao colocarem o ViT em produção: a fome voraz de dados, a rigidez de escala única e o custo computacional proibitivo em altas resoluções. A partir desse diagnóstico, exploraremos as quatro arquiteturas seminais que transformaram o ViT em uma tecnologia industrial: o DeiT e sua destilação com token dedicado, o PVT e sua pirâmide hierárquica para detecção de objetos, o Swin Transformer e seu mecanismo linear de janelas deslocadas com o truque do deslocamento cíclico, e o DINO, demonstrando como ViTs aprendem representações visuais ricas e segmentação semântica sem nenhum rótulo humano. Preparem-se para uma aula transformadora!`
  },
  {
    ...
    component: 'ViTReviewPipelineDiagram',
    title: 'Revisão: A Engenharia do Pipeline Canônico do ViT',
    subtitle: 'Rastreamento de Tensores: do fatiamento 2D à extração final pelo token [CLS]',
    category: 'Revisão Conceitual',
    tag: 'Tensor Tracking',
    notes: `Vamos iniciar com uma revisão passo a passo do fluxo de dados no Vision Transformer canônico, acompanhando o tensor em cada estágio.
...
Por fim, no Estágio 5, descartamos os 196 tokens de patches e extraímos exclusivamente o primeiro vetor, que corresponde ao token [CLS] na saída da última camada: [B, 768]. Esse vetor sintetiza o contexto de toda a imagem e é alimentado em uma cabeça linear MLP com Softmax para prever as probabilidades das classes. Esse é o ViT canônico!`
  },
  {
    id: 3,
    type: 'visual-component',
    component: 'ViTLimitationsDiagnosticDiagram',
    title: 'O Diagnóstico: As 3 Barreiras Críticas do ViT no Mundo Real',
    subtitle: 'Por que a indústria e a pesquisa precisaram evoluir além da arquitetura original de Dosovitskiy et al.?',
    category: 'Situação-Problema do Mundo Real',
    tag: 'Gargalos Críticos',
    notes: `Agora que revisamos com precisão o funcionamento do ViT, precisamos fazer uma pergunta crítica de engenharia: 'Se o Vision Transformer é tão elegante e poderoso, por que ele não substituiu as Redes Convolucionais imediatamente após seu lançamento?'
...
Esses três gargalos exigiram que a comunidade de IA desenvolvesse novas soluções de engenharia.`
  },
  {
    id: 4,
    type: 'visual-component',
    component: 'ViTEvolutionMapDiagram',
    title: 'O Mapa Evolutivo: As Soluções de Engenharia para os Gargalos do ViT',
    subtitle: 'A árvore genealógica dos modelos modernos e os papéis de cada inovação arquitetural',
    category: 'Visão Geral Arquitetural',
    tag: 'Árvore Genealógica',
    notes: `Observem na tela o mapa evolutivo que resume a trajetória dos Vision Transformers nos últimos anos. Cada ramo dessa árvore foi desenhado especificamente para responder a um dos gargalos que acabamos de diagnosticar.
...
Na reta final da aula, veremos também como essas descobertas culminaram nos Masked Autoencoders (MAE), no ConvNeXt — a resposta definitiva das CNNs aos Transformers — e no monumental DINOv2. Vamos mergulhar em cada um desses pilares!`
  ...
  // =========================================================================
  {
    id: 5,
    type: 'visual-component',
    component: 'DeiTDistillationOverviewDiagram',
    title: 'DeiT: Data-Efficient Image Transformers via Destilação',
    subtitle: 'Touvron et al. (Meta/Sorbonne, 2021): Como treinar ViT competitivo usando apenas o ImageNet-1k',
    category: 'Solução de Engenharia • DeiT',
    tag: 'Knowledge Distillation',
    notes: `Vamos iniciar nosso estudo aprofundado pelo DeiT: Data-Efficient Image Transformers.
...
O Student aprende não apenas a acertar a classe correta através dos rótulos reais, mas também é forçado a reproduzir a distribuição de probabilidades gerada pelo professor CNN. Dessa forma, o ViT 'absorve' o viés indutivo convolucional diretamente dos logits do professor, sem que seja necessário modificar os blocos de autoatenção! Com isso, o DeiT-Base saltou de 79.9% para impressionantes 85.2% de acurácia no ImageNet-1k puro.`
  ...
  // =========================================================================
  {
    id: 6,
    type: 'visual-component',
    component: 'DensePredictionBottleneckDiagram',
    title: 'O Dilema das Tarefas Densas: Detecção de Objetos e Segmentação Semântica',
    subtitle: 'Por que a estrutura isotrópica do ViT original falha ao ser conectada a cabeças como FPN, RetinaNet e Mask R-CNN?',
    category: 'Situação-Problema • PVT',
    tag: 'Visão Densa',
    notes: `Superada a barreira da fome de dados com o DeiT, a comunidade enfrentou o segundo grande gargalo: como aplicar Vision Transformers em tarefas de visão densa, especificamente Detecção de Objetos e Segmentação Semântica?
...
O dilema matemático de engenharia era evidente: se tentássemos alimentar uma imagem de resolução padrão de detecção (800 por 800 pixels) em um ViT com stride 4 para obter resolução de um quarto, teríamos 800 dividido por 4 igual a 200 patches por eixo. 200 vezes 200 resulta em 40.000 tokens! A matriz de autoatenção entre 40.000 tokens possui 1.6 bilhão de elementos por cabeça, exigindo mais de 60 gigabytes de VRAM apenas para uma camada! Era matematicamente inviável rodar um ViT tradicional em FPN.`
  },
  {
    id: 7,
    type: 'visual-component',
    component: 'PVTArchitectureDiagram',
    title: 'Pyramid Vision Transformer (PVT) e Spatial-Reduction Attention (SRA)',
    subtitle: 'Wang et al. (2021): Transformando o ViT em um backbone hierárquico com redução espacial de chaves e valores',
    category: 'Solução de Engenharia • PVT',
    tag: 'Hierarquia & SRA',
    notes: `Para resolver de forma elegante o dilema da visão densa, Wang e seus colaboradores propuseram no artigo seminal de 2021 o PVT: Pyramid Vision Transformer.
...
Vejam a matriz de atenção Q vezes K transposto: sua dimensão deixa de ser N por N e passa a ser N por (N dividido por R ao quadrado). A complexidade cai por um fator brutal de R ao quadrado! No Estágio 4, onde a resolução já é pequena (H sobre 32), R é configurado como 1, retornando à autoatenção global clássica. O PVT foi o primeiro backbone puramente baseado em atenção a superar a ResNet-50 em detecção de objetos no benchmark COCO!`
  },
  {
    id: 8,
    type: 'interactive',
    component: 'PVTSimulatorLab',
    title: '🧪 Lab 1: Simulador de Pirâmide Visual e Spatial-Reduction Attention',
    subtitle: 'Ajuste a resolução da imagem (H×W) e a taxa de redução R_i e veja o consumo de VRAM e FLOPS em cada estágio',
    category: 'Laboratório Interativo',
    tag: 'Simulação PVT',
    notes: `Chegamos ao nosso primeiro laboratório interativo, focado na mecânica e viabilidade de hardware do Pyramid Vision Transformer.
...
Vejam o impacto prático para uma imagem de 224 por 224 no Estágio 1, com 3.136 patches: enquanto o MSA global executa 19.6 milhões de operações por canal no termo de atenção, o W-MSA do Swin consome apenas 300 mil operações — uma redução de mais de 64 vezes! Isso tornou o Swin escalável para resoluções de 4K e 8K sem sofrer gargalo quadrático.`
  },
  {
    id: 10,
    type: 'visual-component',
    component: 'SwinShiftedWindowDiagram',
    title: 'Shifted Windows (SW-MSA): Interligando Janelas sem Custo Global',
    subtitle: 'Deslocando a grade de janelas por (⌊M/2⌋, ⌊M/2⌋) para capturar relações entre bordas',
    category: 'Solução de Engenharia • Swin',
    tag: 'Shifted Windows',
    notes: `Entretanto, qualquer engenheiro atento identificará imediatamente uma fraqueza crítica no W-MSA puro: se a autoatenção fica estritamente confinada dentro de cada janela quadrada de 7 por 7, os patches que estão em janelas vizinhas NUNCA conversam entre si! A rede perderia a capacidade de modelar objetos grandes cujas partes cruzam as fronteiras das janelas, sacrificando o poder representacional global do Transformer.
...
Ao alternar sistematicamente entre W-MSA e SW-MSA bloco a bloco, a informação se propaga rapidamente por toda a imagem camada após camada, criando conexões espaciais globais enquanto mantém rigorosamente a complexidade linear O(N) em cada bloco individual. Foi uma solução de pura genialidade arquitetural!`
  },
  {
    id: 11,
    type: 'visual-component',
    component: 'SwinCyclicShiftDiagram',
    title: 'Cyclic Shift & Masked Attention: O Golpe de Mestre da Implementação',
    subtitle: 'Como processar janelas deslocadas de tamanhos irregulares em um batch uniforme sem padding',
    category: 'Formalismo de Engenharia • Swin',
    tag: 'Cyclic Shift & Mask',
    notes: `Mas como todo grande projeto de engenharia de software e inteligência artificial, uma ideia conceitual brilhante muitas vezes encontra uma barreira brutal de implementação no hardware.
...
No Passo 4, após o cálculo da atenção vetorizada em GPU, eles aplicam o Reverse Cyclic Shift (rolagem inversa), devolvendo os patches às suas coordenadas geométricas originais. O resultado? Execução 100% paralela na GPU com zero padding e máxima eficiência de silício!`
  },
  {
    id: 12,
    type: 'visual-component',
    component: 'DINOOverviewDiagram',
    title: 'DINO: Self-Supervised Vision Transformers sem Rótulos',
    subtitle: 'Caron et al. (Meta AI, ICCV 2021): Aprendendo representações visuais puras por auto-destilação sem pares negativos',
    category: 'Solução de Engenharia • DINO',
    tag: 'Self-Supervision',
    notes: `Até agora, todos os modelos que estudamos — ViT, DeiT, PVT e Swin — foram treinados sob o paradigma do aprendizado supervisionado: milhões de imagens rotuladas manualmente por seres humanos com rótulos categóricos como 'cachorro', 'carro' ou 'avião'.
...
A função de perda é uma Cross-Entropy simples entre o Softmax do Aluno e o Softmax do Professor. Notem uma diferença crucial em relação a métodos auto-supervisionados clássicos como SimCLR ou MoCo: o DINO NÃO precisa de pares negativos (negative pairs), dispensando filas de memória gigantescas e simplificando drasticamente o pipeline!`
  },
  {
    id: 13,
    type: 'visual-component',
    component: 'DINOMouseMechanicsDiagram',
    title: 'A Mecânica do DINO: Prevenção de Colapso e a Propriedade Emergente',
    subtitle: 'Momentum Encoder (EMA), Centering, Sharpening e a descoberta de máscaras de segmentação sem supervisão',
    category: 'Teoria e Descoberta • DINO',
    tag: 'Emergência Semântica',
    notes: `No entanto, qualquer pesquisador de aprendizado auto-supervisionado sabe que treinar uma rede siamesa sem pares negativos esbarra em uma armadilha fatal: o Colapso de Representação (Representation Collapse). O modelo pode simplesmente aprender a saída constante — por exemplo, prever um vetor idêntico de zeros ou um único valor para qualquer imagem que entre —, obtendo perda zero sem aprender absolutamente nada sobre o mundo visual!
...
Sem jamais ter visto uma única caixa delimitadora, sem nunca ter recebido um único rótulo de classe e sem nenhuma anotação de máscara de segmentação, o ViT aprendeu por conta própria a fronteira precisa entre o objeto e o fundo da cena. E cada cabeça de atenção do DINO especializou-se espontaneamente em partes anatômicas coerentes (cabeça, patas, asas, rodas). Foi uma das maiores demonstrações do poder representacional dos Vision Transformers na história da inteligência artificial!`
  },
  {
    id: 14,
    type: 'interactive',
    component: 'DINOAttentionViewerLab',
    title: '🧪 Lab 2: Visualizador de Mapas de Atenção: DINO vs ViT Supervisionado',
    subtitle: 'Explore visualmente como a atenção auto-supervisionada isola contornos semânticos limpos em comparação com a supervisão tradicional',
    category: 'Laboratório Interativo',
    tag: 'Simulação DINO',
    notes: `Nosso segundo laboratório interativo permite que vocês visualizem e sintam exatamente essa propriedade emergente que acabamos de estudar.
...
Esse comportamento não foi programado manualmente por nenhum desenvolvedor. Ele emergiu do objetivo puramente auto-supervisionado de prever o todo a partir das partes. É por isso que modelos como o DINO e o DINOv2 tornaram-se a fundação de quase todos os sistemas modernos de visão computacional.`
  ...
  // =========================================================================
  // BLOCO 6: MODELOS MAJORITÁRIOS, CONVNEXT E GUIA DE SELEÇÃO INDUSTRIAL (Slides 15 a 17)
  // =========================================================================
  {
    id: 15,
    type: 'visual-component',
    component: 'MAEAndDINOv2Diagram',
    title: 'MAE & DINOv2: As Fronteiras Modernas do Pré-Treinamento Visual',
    subtitle: 'Reconstrução pixel-level com 75% de máscara (He et al.) e representações universais em escala de bilhão (DINOv2)',
    category: 'Fronteiras Modernas',
    tag: 'MAE & DINOv2',
    notes: `Conforme nos aproximamos do estado da arte contemporâneo, precisamos examinar duas tecnologias que levaram o pré-treinamento de Vision Transformers a um novo patamar: o MAE (Masked Autoencoders) e o DINOv2.
...
O resultado do DINOv2 foi um marco: as representações geradas por seus modelos congelados (Frozen Features) são tão ricas que superam modelos supervisionados com fine-tuning completo em classificação, profundidade monocular e segmentação de instâncias!`
  },
  {
    id: 16,
    type: 'visual-component',
    component: 'ConvNeXtEvolutionDiagram',
    title: 'ConvNeXt: A Modernização da Convolução nos Anos 2020',
    subtitle: 'Liu et al. (CVPR 2022): Como atualizar uma ResNet-50 aplicando as lições de design do Swin Transformer',
    category: 'O Retorno das CNNs • ConvNeXt',
    tag: 'A ConvNet for the 2020s',
    notes: `Diante de todas essas inovações fantásticas dos Vision Transformers, em 2022 a comunidade acadêmica e industrial começou a se perguntar: 'Será que as Redes Convolucionais estão realmente obsoletas, ou elas apenas ficaram para trás porque ninguém atualizou suas práticas de design com as lições modernas aprendidas com os Transformers?'
...
O ConvNeXt-Tiny igualou e até superou o Swin-Tiny em acurácia e FLOPs, mantendo a simplicidade de inferência, o throughput superior e a facilidade de deploy em chips móveis (Edge/NPU) que consagram as CNNs. A lição de engenharia foi clara: o segredo do sucesso dos Transformers não era apenas o mecanismo de atenção, mas sim o design macroscópico dos blocos e os protocolos modernos de treinamento!`
  },
  {
    id: 17,
    type: 'interactive',
    component: 'ViTAdvancedQuizLab',
    title: '🧪 Lab 3: Quiz de Fixação & Matriz de Decisão Arquitetural',
    subtitle: 'Teste seus conhecimentos sobre DeiT, PVT, Swin, DINO e consulte o guia de seleção de modelos para projetos reais',
    category: 'Laboratório Final & Decisão',
    tag: 'Avaliação & Guia',
    notes: `Chegamos ao encerramento da nossa Aula 5 com o nosso terceiro laboratório interativo! Este módulo foi desenhado em duas partes essenciais para consolidar a jornada de aprendizado de vocês.
